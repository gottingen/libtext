...
function(carbin_cc_library)
    ...
    if (NOT CARBIN_CC_LIB_IS_INTERFACE)
        add_library(${CARBIN_CC_LIB_NAME}_OBJ OBJECT ${CARBIN_CC_LIB_SOURCES} ${CARBIN_CC_LIB_HEADERS})
        ...
        target_include_directories(${CARBIN_CC_LIB_NAME}_OBJ ${CARBIN_INTERNAL_INCLUDE_WARNING_GUARD}
                PUBLIC
        ...
        target_compile_definitions(${CARBIN_CC_LIB_NAME}_OBJ
                PUBLIC
                ${CARBIN_CC_LIB_DEFINITIONS}
                )
        set_property(TARGET ${CARBIN_CC_LIB_NAME}_OBJ PROPERTY POSITION_INDEPENDENT_CODE 1)
        if (BUILD_STATIC_LIBRARY)
            add_library(${CARBIN_CC_LIB_NAME}-static STATIC $<TARGET_OBJECTS:${CARBIN_CC_LIB_NAME}_OBJ>)
        ...
        if (BUILD_SHARED_LIBRARY)
            add_library(${CARBIN_CC_LIB_NAME}-shared SHARED $<TARGET_OBJECTS:${CARBIN_CC_LIB_NAME}_OBJ>)
            set_target_properties(${CARBIN_CC_LIB_NAME}-shared PROPERTIES OUTPUT_NAME ${CARBIN_CC_LIB_NAME} CLEAN_DIRECT_OUTPUT 1)
            add_library(${CARBIN_CC_LIB_NAMESPACE}::${CARBIN_CC_LIB_NAME} ALIAS ${CARBIN_CC_LIB_NAME}-shared)
        endif ()
    ...
    if (CARBIN_CC_LIB_PUBLIC)
        if (BUILD_SHARED_LIBRARY AND BUILD_STATIC_LIBRARY)
            install(TARGETS ${CARBIN_CC_LIB_NAME}-shared ${CARBIN_CC_LIB_NAME}-static
